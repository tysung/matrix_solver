...
cd /import/home/tsung/CODE/sp7_cift-internal/temp/all-slices-xc7s25csga324-1-2021-04-15T17-48-43/1-mux1/build-00000
#
...
foreach cell [get_cells harness/test_region_i/* -filter {REF_NAME=="plb"}] {
    ...
    set_property MANUAL_ROUTING SLICEL $cell_slice
    # remove any BEL which is not LUT
    ...
    foreach bus_index {0 1 2 3} {
        ...
        #
        #append O6_2_mux " $cell_slice/[lindex "AOUTMUX BOUTMUX COUTMUX DOUTMUX" $bus_index]:06"
        #
    ...
    set_property SITE_PIPS {AOUTMUX:O6 BOUTMUX:O6 COUTMUX:O6 DOUTMUX:O6} $cell_slice
}
#for {set i 0} {$i<[llength $site_pins]} {incr i} {
#    puts "$i = [get_nets -of_objects [lindex $site_pins $i]] [lindex $site_pins $i]" 
#}
...
#}
# place those cells removed from the PLB
