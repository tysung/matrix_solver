...
cd /import/home/tsung/CODE/sp7_cift-internal/temp/all-slices-xc7s25csga324-1-2021-05-03T11-13-33/1-regtest5/build-00000
#
...
foreach cell [get_cells harness/test_region_i/* -filter {REF_NAME=="plb"}] {
    ...
    #
    set lut_cell [get_cells -of $cell_slice -filter {PRIMITIVE_GROUP==LUT}]
    if {[llength $lut_cell]  > 0} {unplace_cell $lut_cell}
    #
    ...
    set_property MANUAL_ROUTING [get_property SITE_TYPE $cell_slice] $cell_slice
    # set CLK input to use INVERTED CLK signal for internal FF
    set_property SITE_PIPS { CLKINV:CLK_B SRUSEDMUX:IN CEUSEDMUX:IN A5FFMUX:IN_B B5FFMUX:IN_B C5FFMUX:IN_B D5FFMUX:IN_B AOUTMUX:A5Q BOUTMUX:B5Q COUTMUX:C5Q DOUTMUX:D5Q AFFMUX:O6 BFFMUX:O6 CFFMUX:O6 DFFMUX:O6 } $cell_slice
    for {set i 0} {$i<[llength $bel_cell]} {incr i} {
    ...
    for {set i 0} {$i<[llength $lut_cell]} {incr i} {
        place_cell [lindex $lut_cell $i] $cell_slice
     }
}
place_design
route_design
