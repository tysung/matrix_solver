...
my $args; 
my $message;
my $QA_HOME = "/import/home/tsung/CIFT_WEEK";
#
...
my $qa_result = "${QA_HOME}/week/cift-internal/RESULT";
my $qa_result_disk = "/projects/cift/builds/tsung/WEEK_RESULT";
my $build_script = "${QA_HOME}/week/cift-internal/SCRIPT";
my $qa_report = "${QA_HOME}/LastTest.log";
my $today_excel = "${qa_golden_db}/week_${date}_db.xlsx";
...
if( -e "${QA_HOME}/week/cift-internal" ) {
	chdir("${QA_HOME}/week/cift-internal");
...
if(1==1) {
	...
	# build Python Env
	#
	$exit_code = system('./install.sh');
	if ($exit_code != 0) {
		print "install the python env failure .... \n";
		exit(1);
	}
	# virtex6 slices need unlimited stack in process
	# $exit_code = system('ulimit -s unlimited');
	#if ($exit_code != 0) {
	#	print "install the python env failure .... \n";
	#	exit(1);
	#}
	...
	#
	#system("rm -rf ${qa_result_disk}/*");
	#system("ln -s ${qa_result_disk} ${qa_result}");
	...
	foreach my $family (keys %$test_list) {
		...
		#}
		#
		foreach my $type (keys %{$test_list->{$family}}) {
		#
			print " [${family}] - Build Tests ..... \n";
			system("mkdir -p ${qa_result}/${family}/${type}");
			my $build_f = $test_list->{$family}->{$type}->{'script'};
			print " [${family}/${type}/${build_f}] - Build This Test ..... \n";
			my $out_f = "${qa_result}/${family}/${type}/LOG";
			system("./ift --batch ${build_f} |& tee $out_f");
			$test_list->{$family}->{$type}->{'out_f'} = $out_f;
			# check if no files were generated
			my $file_num = `dir -ag tests | wc -l`;
			if ($file_num > 3) {
				system("mv -f tests/* ${qa_result}/${family}/${type}");
			}	
			$file_num = `dir -ag temp | wc -l`;
			if ($file_num > 3) {
				system("mkdir -p ${qa_result}/${family}/${type}/temp");
				system("mv -f temp/* ${qa_result}/${family}/${type}/temp/");
			}
		}
	}
	##
	...
	foreach my $family ( keys %$test_list ) {
		...
		foreach my $type (keys %{$test_list->{$family}}) {
			...
			# if( glob("*.ift-tg") ) - anyway, check for each file is needed!
			{
				...
				my $path = dirname($test_list->{$family}->{$type}->{'out_f'});
				#$path = "${path}/${type}";
				# there are new test case which can fail in 1st time
				my $all_tests = $test_list->{$family}->{$type}->{'tests'};
				...
				$test_list->{$family}->{$type}->{'pass_fail'} = $DB->{'check'}->{'pass_fail'};
...
sub all_db_update {
	...
	foreach my $family (keys %$test_list) {
		#my $test_types = $test_list->{$family}->{'types'};
		foreach my $type (keys %{$test_list->{$family}}) {
			if((!defined $test_list->{$family}) || (!defined $test_list->{$family}->{$type})) {
				next;
			}
	...
	foreach my $Y_name (@{$db->{'Y_name'}}) {
		...
		my $result = $test_list->{$family}->{$type}->{'pass_fail'};
		my @out_str = ();
		# debug --
		if( ! defined $result ) {
			print("There is no result for this build test ${family}/${type} \n");
		}
...
sub result_check_type {
	...
	# 'no-bitgen' test name from "build-script", DB->{golden,result} is .bit file without '.bit'
...
sub gen_test_list {
	...
	#system("rm -rf $build_script");
	#system("mkdir -p $build_script");
	my @build_files = glob "./testgroups/virtex{5,6}/build-tests-*.py";
	#my @build_files = glob "./test/weekly/build-scripts/build-tests-virtex6*.py";
	foreach my $build_file (@build_files) {
		my $family = '';
		my $type = '';
		my $path = dirname($build_file);
		...
		$abs_path =~ s/testgroups/RESULT/g;
		$abs_path = $abs_path . '/' . "${type}/LOG";
		$test_list->{$family}->{$type} = { 
				  'tests' => $db->{$type}->{'tests'}, 
				  'tg_name' => $db->{$type}->{'tg_name'}, 
				  'part' => undef, 
				  'out_f' => $abs_path, 
				  'script' => $build_file 
		};

		# modify the parallel job number
		#
		@words = split('/', $build_file);
		#if( $words[2] eq 'virtex6' ) {
		#	next;
		#}
		my @data = `cat $build_file`;
		#chomp(@data);
		@data = map {/^parallel_jobs/ ? "parallel_jobs = 16\n" : $_} @data;
		if (($words[3] =~ "configmem") && ($words[2] eq 'virtex5')) {
			@data  = map {  /^part/ ? "part = \'xc5vlx110tff1136-1\'\n" : $_ } @data;
		}
		open(F, ">${build_file}") or die "Fail to open ${build_file} \n";
		print F @data;
		close(F);
	...
	##
	#
	# trim family in order to finish in 48 hours
	#
	my $trim_family = ['series7', 'virtex6', 'virtex5', 'virtex4', 'spartan3', 'virtex2', 'virtex2p'];
	# remove 'virtex6' from the array
	$trim_family = ['series7', 'virtex4', 'spartan3', 'virtex2', 'virtex2p'];
	my $trimed =  map { delete $test_list->{$_} } @{$trim_family}; # trim the family
	#
	# trim type if needed!
	#
	foreach my $family ( keys %$test_list ) {
		#$trim_family = ['all', 'slices', 'brams', 'configmem', 'harness', 'interconnect'];
		$trim_family = ['all'];
		my $trimed =  map { delete $test_list->{$family}->{$_} } @{$trim_family}; # trim the type 
	}
	#
	open(DUMPFILE, '>', $tests_dump) or die "Couldn't open file for writing $tests_dump - $!. \n";
	$Data::Dumper::Indent = 1;
...
sub match_test_bitgen {
	...
	foreach my $family ( keys %$test_list ) {
		...
		foreach my $type (keys %{$test_list->{$family}}) {
			...
			$test_list->{$family}->{$type}->{'tests'} = $all_tests;
		}
	}
}
...
sub result_move_type {
	...
	foreach my $family (keys %$test_list) {
		...
		foreach my $type (keys %{$test_list->{$family}}) {
			...
			foreach my $test (@{$test_list->{$family}->{$type}->{'tests'}}) {
			...
			my $tg_name = $test_list->{$family}->{$type}->{'tg_name'};
			if( -e $tg_name ) {
				system("mv -f $tg_name $type/");
			}
			my $out_f = "${qa_result}/${family}/${type}/LOG";
			if ( (! defined $test_list->{$family}->{$type}->{'out_f'}) && (-e $out_f) ) {
				$test_list->{$family}->{$type}->{'out_f'} = $out_f;
				system("mv -f $out_f $type/");
			}
		}
		chdir("..");
	}
