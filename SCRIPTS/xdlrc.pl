...
#   http://scholar.lib.vt.edu/theses/available/etd-09112002-143335
	...
	$::build_number = "7"; $ENV{"XIL_DRM_EXCLUDE_ARCS"} = $ENV{"XIL_TEST_ARCS"} = 1; $::version_suffix = " [XIL_TEST_ARCS]";
	# build number 7: 7/11/2015 added Spartan3; no format change
	# build number 6: 11/26/2014 no code changes; building ISE 14.5 Zynq with test arcs
	...
	sub preprocess_architecture($@) {
		...
		# analyze the first device for primitive definitions
		{
			...
			$line = <XDLRC>;
			$line =~ /^# (\S+) "(.+)"$/;
			($::version_number, $::version_string) = ($1, $2 . $::version_suffix);
...
#print $tiles[-1] . "\n";
					...
					# tysung - $wire_indexes = \$tile_type_wire_index{$tile_type_name};
				...
				# process wire declarations
					...
					# tysung - wire found issue
					#my %hash = %$wire_indexes;
				...
				# process pip declarations
				} elsif($line =~ /^\s*\(pip  (\w+) (==|=>|=-|->) (\w+)( \(_ROUTETHROUGH-(\S+)-(\S+) (\S+)\))?\)/) {
					...
					# extract the variables
					my ($wire_name_1, $direction, $wire_name_2) = ($1, $2, $3);
					my ($is_routethrough, $rt_source_pin, $rt_sink_pin, $rt_primitive_def) = ($4, $5, $6, $7);
...
#		}
		...
		my $index = 0;
		# tysung, this tile_type_index dependency is removed
		...
		foreach my $device (sort order_devices keys %device_tiles) {
			...
			foreach my $tile_entry (@{$device_tiles{$device}}) {
				...
				$tile_entry = join ",", $tile_index, $tile_row, $tile_col, $tile_name, $tile_type_name, $tile_site_count; # tysung, remove tile type index dependency
			}
		}

		# write out the device digests
		# write all the instance tiles of this device. It is not a problem to be parallelized.
		foreach my $device (sort order_devices keys %device_tiles) {
		...
		# tysung, in order to parallelized, read in the PM dump file first and consolicate all tile_type_hash
		# After write out the "architecture.digest", write out the PM dump file again for next run.
		# there might be some "tile_type" already been processed already.
		my $DB = read_dump_arch_digest(
			$architecture,
			\@tile_type_name,
			\%tile_type_all_arcs,
			\%tile_type_irregular_arcs,
		...
		@tile_type_name = @{$DB->{'tile_type_name'}};
		%tile_type_all_arcs = %{$DB->{'tile_type_all_arcs'}};
		%tile_type_irregular_arcs = %{$DB->{'tile_type_irregular_arcs'}};
		%tile_type_routethrough_arcs = %{$DB->{'tile_type_routethrough_arcs'}};
		%tile_type_wire_index = %{$DB->{'tile_type_wire_index'}};
		...
		for($index = 0; $index < scalar @tile_type_name; $index++) {
			...
			print ARCH "$tile_type_name;" . (join ",", @wire_names) . ";"; # tysung, remove tile type index dependency
...
print "    $tile_type_name: $source_wire_name > $sink_wire_name\n";
	...
	sub read_dump_arch_digest($@) {
		# extract the parameters
		my $architecture = shift;
		my $tile_type_name = shift;
		my $tile_type_all_arcs_ref = shift;
		my $tile_type_irregular_arcs_ref = shift;
		my $tile_type_routethrough_arcs_ref = shift;
		my $tile_type_wire_index_ref = shift;

		my $filename = "${architecture}_dump.pm";
		my $DB = {
			'tile_type_name'              => [],
			'tile_type_irregular_arcs'    => {},
			'tile_type_routethrough_arcs' => {},
		...
		if (-e $filename) {
			open(FILE, $filename) or die "Couldn't open file for reading $filename - $!. \n";
			my $sep = $/;
			undef $/;
			$DB = eval <FILE>;
			close FILE;
		...
		my $new_tile_type_name_ref = [@$tile_type_name, @{$DB->{'tile_type_name'}}];
		my %hash = map { $_ => 1 } @$new_tile_type_name_ref;
		$new_tile_type_name_ref = [ keys %hash ];
		my $new_tile_type_all_arcs_ref = {%$tile_type_all_arcs_ref, %{$DB->{'tile_type_all_arcs'}}};
		my $new_tile_type_irregular_arcs_ref = {%$tile_type_irregular_arcs_ref, %{$DB->{'tile_type_irregular_arcs'}}};
		my $new_tile_type_routethrough_arcs_ref = {%$tile_type_routethrough_arcs_ref, %{$DB->{'tile_type_routethrough_arcs'}}};

		my $new_tile_type_wire_index_ref = merge_hash_hier($tile_type_wire_index_ref, $DB->{'tile_type_wire_index'});
		#my $new_tile_type_wire_index_ref = {%$tile_type_wire_index_ref, %{$DB->{'tile_type_wire_index'}}};

		$DB = {};
		$DB->{'tile_type_name'} = $new_tile_type_name_ref;
		$DB->{'tile_type_all_arcs'} = $new_tile_type_all_arcs_ref;
		$DB->{'tile_type_irregular_arcs'} = $new_tile_type_irregular_arcs_ref;
		$DB->{'tile_type_routethrough_arcs'} = $new_tile_type_routethrough_arcs_ref;
		$DB->{'tile_type_wire_index'} = $new_tile_type_wire_index_ref;

		open(DUMPFILE, '>', $filename) or die "Couldn't open file for writing $filename - $!. \n";
		$Data::Dumper::Indent = 1;
		$Data::Dumper::Useqq = 1;
		print DUMPFILE Data::Dumper->Dump( [$DB], [qw(DB)] );
		close(DUMPFILE) or die "Couldn't close the file $filename \n";
	...
	sub merge_hash_hier($@) {
		...
		my $result_hash = {};
		my $arr_rf = [keys %$rf_hash];
		my $arr_db = [keys %$db_hash];

		if(scalar @$arr_db == 0) {
			return $rf_hash;
		} elsif(scalar @$arr_rf == 0) {
			return $db_hash;
		} else {
			my $arr_all = [@$arr_db, @$arr_rf];
			my %hash = map { $_ => 1 } @$arr_all;
			$arr_all = [ keys %hash ];
			foreach my $tile_type (@$arr_all) {
				my $rf_tile_type_hash = {};
				if(exists $rf_hash->{$tile_type}) {
					$rf_tile_type_hash = $rf_hash->{$tile_type};
				}
				my $db_tile_type_hash = {};
				if(exists $db_hash->{$tile_type}) {
					$db_tile_type_hash = $db_hash->{$tile_type};
				}
				$result_hash->{$tile_type} = {%$db_tile_type_hash, %$rf_tile_type_hash};
				# assign new wire index based on the merge hash
				my $all_keys = [sort keys %{$result_hash->{$tile_type}}];
				my $num = 0;
				foreach my $key (@$all_keys) {
					$result_hash->{$tile_type}->{$key} = $num++;
				}
			}
			return($result_hash);
		}
...
#}
				...
				my ($tile_type_name, $tile_type_wires, $tile_type_regular_arcs, $tile_type_irregular_arcs,
					$tile_type_routethrough_arcs) = split ";", $line;
				# tysung, my ($tile_type_index, $tile_type_name) = split ",", $tile_type_header;
				...
				# tysung - tile type index restored here
				my $tile_type_index = -1;
				if( ! defined $::tile_type_name_to_tile_type_index{$tile_type_name} ) {
					$tile_type_index = keys %::tile_type_name_to_tile_type_index;
					...
					$tile_type_index = $::tile_type_name_to_tile_type_index{$tile_type_name};
				}
	...
	sub read_device_digest($) {
		...
		foreach my $line (@lines) {
			#push @::tiles, $line;
			# tysung - restored tile_type_index
			#my ($tile_index, $tile_row, $tile_col, $tile_name, $tile_type_name, $tile_site_count) = split ",", $line;
			my @tmp = split ",", $line;
			my $tile_index = $tmp[0];
			my $tile_name = $tmp[3];
			my $tile_type_name = $tmp[4];
			my $tile_type_index = $::tile_type_name_to_tile_type_index{$tile_type_name};
			$::tile_name_to_tile_index{$tile_name} = $tile_index;
			$::tile_name_to_tile_type_index{$tile_name} = $tile_type_index;
			# overwrite $tile_type_name with $tile_type_index and push it into @::tiles
			$tmp[4] = $tile_type_index;
			$line = join(",", @tmp);
			push @::tiles, $line;
		}
		close DEVICE;
...
#print "" . (join ",", sort keys %{$::tile_type_wire_name_to_wire_index{$tile_type_index}}) . "\n";
			...
			# process pinwire statements
				...
				if(!defined $wire_index) {
					$wire_index = -1;
				} elsif($pinwire_type eq "input") {
...
#print LOG "NOTE: defining $compact_segment{$compact_segment}:$compact_segment\n" if $found;
			...
			} elsif($line =~ /^\s*\(pip  (\w+) (==|=>|=-|->) (\w+)( \(_ROUTETHROUGH-(\S+)-(\S+) (\w+)\))?\)/) {
				# ==   Bidirectional, unbuffered
				# =>   Bidirectional, buffered in one direction
				# =-   Bidirectional, buffered in both directions
				# ->   Directional, buffered
...
#print "$tile_type_name: \n";
			...
			foreach my $irregular_arc (keys %$irregular_arcs) {
				...
				if( !defined $source_wire_index || !defined $sink_wire_index ) {
					print LOG "Error: [$tile_type_name] $source_wire_name > $sink_wire_name \n";
				}
...
#		extract_tile_types($architecture);
	...
	# preprocess a package for a custom architecture
	sub preprocess_custom_package($@) {
		my $architecture = shift;
		my @devices = @_;

		# iterate over all specified devices
		my $digest = "";
		foreach my $device (@_) {
			...
			# identify all of the primitive sites in the custom xdlrc
			print "Processing $device...  ";
			my %sites = ();
			...
			# arbitrarily set the speed grade to 0 and reuse the device name as the package name
			$digest .= "$device:0:$device;";
	...
	# provide usage information if the user has specified no parameters
	if(scalar @ARGV == 0) {
		print "Usage: \n"
			. "    $0 [xdlrc] [packages] [preprocess] [process] <architecture>\n"
			. "    $0 [packages] [preprocess] [process] custom:<architecture>:<device>[,<device>]*\n"
			. "\n"
			. "    [xdlrc]: Invokes the ISE xdl tool to generate full XDLRC files.\n"
			. "    [packages]: Invokes the ISE partgen tool to generate full package information.\n"
			. "    [preprocess]: Preprocesses the architecture and devices.\n"
			. "    [process]: Processes the architecture and devices and generates Torc databases.\n"
			. "\n"
			. "    <architecture> tells $0 which architecture(s) from devices.pl to process.\n"
			. "    If custom:<architecture>:<device> is specified, the file named "
			. "<device>.conns.xdlrc.gz must exist.\n"
			. "\n";
		exit;
	}
	...
	my ($xdlrc, $packages, $preprocess, $process, $development, $tagm12, $isirrex, $virtex, 
		...
		$virtex7l, $kintex7, $kintex7l, $artix7, $zynq7000, $spartan3, $spartan3e, $spartan6, 
		$spartan6l)
	...
	foreach my $arg (@ARGV) {
		...
		$spartan3 = true if $arg =~ /\bspartan3\b/i;
		$spartan3e = true if $arg =~ /\bspartan3e\b/i;
		$spartan6 = true if $arg =~ /\bspartan6\b/i;
		$spartan6l = true if $arg =~ /\bspartan6l\b/i;
	}

	# determine if the user has requested a custom device (not defined in devices.pl)
	my $args = join " ", @ARGV;
	my ($custom, $architecture, @devices) = (false, undef, ());
	($custom, $architecture, @devices) = (true, $1, (split ",", $2)) if ($args =~ /\bcustom:(\w+):([\w,]+)\b/);
	if($development) {
		print "NOTE: Use of the Development architecture is deprecated.\n" .
			"    Please use: $0 <functions> custom <architecture> <device>\n";
	}

	# generate device XDLRC as requested
	if($xdlrc && $custom) {generate_xdlrc($architecture, @devices); $xdlrc=false;}
	#print "NOTE: Cannot generate XDLRC for custom architectures.\n" if $xdlrc && $custom;
	...
	generate_xdlrc("spartan3", @devices::spartan3) if $xdlrc && $spartan3;
	...
	if($packages && $custom) {preprocess_packages($architecture, @devices); $packages=false;}
	...
	preprocess_packages("spartan3", @devices::spartan3) if $packages && $spartan3;
	...
	if($preprocess && $custom) {preprocess_architecture($architecture, @devices); $preprocess=false;}
	...
	preprocess_architecture("spartan3", @devices::spartan3) if $preprocess && $spartan3;
	...
	if($process && $custom) {process_wiring($architecture, @devices); $process=false;}
	...
	process_wiring("spartan3", @devices::spartan3) if $process && $spartan3;
